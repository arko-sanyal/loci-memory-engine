...
| CARD integration with `loci_engine` heat | 04 | Not started | Proposed to Codex via the coordination bus (task e58b100f, docs/superpowers/plans/2026-09-10-loci-versioning-provenance-card.md) — `rag/card.py` still ranks by raw metadata dates, not real heat |
| Fact versioning + provenance boundary | 08 | Not started | Same proposed task as above — supersedes/valid_from/valid_until/source_trust on `qsymprev`, and the `UserStated` type boundary closing a real memory-poisoning vulnerability found this session |
